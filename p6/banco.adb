...
package body Banco is 
   ...
   procedure Agregar_Cuenta (
      B : in out Banco_Array; 
      C : in Cuenta.Cuenta_bancaria
      ) is
      I : Integer;
   begin
      for I in 1 .. 10 loop
   ...
   begin
      ...
      if Cuenta.Consultar_Saldo(C1) >= Monto then
         ...
         raise Constraint_Error with "Saldo insuficiente para realizar la transferencia.";
      end if;
   end Transferir;

   function Consultar_Cuenta (
      B : in Banco_Array; 
      T : in Unbounded_String
      ) return Float is
      I : Integer;
   begin
      for I in 1 .. 10 loop
   ...
   function Consultar_Saldo_Banco (
      B : in Banco_Array
      ) return Float is
      Total : Float := 0.0;
   begin
